  ...
  return (
    <div className='flex-1 h-0 grid grid-cols-1 content-start lg:grid-cols-3 xl:grid-cols-4 gap-3'>
        {INTEGRATION_LIST_ITEMS.map((item) => (
            <Card key={item.id}>
                 <CardContent className="flex flex-col p-5 gap-2 pl-0">
            <div className="flex w-full justify-between items-start gap-x-20">
              <div className="">
                <div className="w-20 h-9 relative pr-5">
                  <Image
                    sizes="100vw"
                ...
                <h2 className="font-bold capitalize pl-2">{item.name}</h2>
              </div>
              <IntegrationTrigger
            ...
            <CardDescription className='pl-2'>{item.description}</CardDescription>
          </CardContent>
            </Card>
        ))}
    </div>
